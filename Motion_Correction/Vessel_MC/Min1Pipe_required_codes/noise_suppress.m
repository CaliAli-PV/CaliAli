function [m_out] = noise_suppress(m_in,Fs, nflag, filename)
...
%   Jinghao Lu, 11/18/2019
    ...
    if overwrite_flag || nflag == 1
        ...
        for ib = 1: nbatch
            data = m_in(:, idbatch(ib): idbatch(ib + 1) - 1, :);
            ...
            %%% update data %%%
            m_in(:, idbatch(ib): idbatch(ib + 1) - 1, :) = data;
            imax = max(imax, max(max(max(data))));
            imin = min(imin, min(min(min(data))));
        end
        
    
        m_out = single(mat2gray(m_in));
