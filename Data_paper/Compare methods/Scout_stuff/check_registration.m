...
if sum(sizeFixed == sizeMoving) ~= dims
    warning('Provided image data is not of the same size: padding images to match (2d only)')
    dim1=sizeFixed(1)-sizeMoving(1);
    dim2=sizeFixed(2)-sizeMoving(2);
    if dim1>0
        registration.moving=[registration.moving;zeros(dim1,size(registration.moving,2))];
    elseif dim1<0
        registration.fixed=[registration.fixed;zeros(abs(dim1),size(registration.fixed,2))];
    end    
    if dim2>0
        registration.moving=[registration.moving,zeros(size(registration.moving,1),dim2)];
    elseif dim2<0
        registration.fixed=[registration.fixed,zeros(size(registration.fixed,1),abs(dim2))];
    end      
    sizeFixed=size(registration.fixed);
    sizeMoving=size(registration.moving);
end
