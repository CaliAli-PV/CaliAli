function [w,mu,sigma,error,f]=gmm_classification(X,side,num_iter)
% construct parameters for gaussian mixture model using data from X
...
modelfun=@(b,x) b(1)*normpdf(x,b(2),b(3))+(1-b(1))*normpdf(x,b(4),b(5));
...
try
    warning('');
beta=nlinfit(x1,f,modelfun,[.5,model_min,.1,model_max,.1]);
...
try
warning('');
init_beta=[unifrnd(.25,.75),unifrnd(model_min,model_max),unifrnd(.001,.5),unifrnd(model_min,model_max),unifrnd(.001,.5)];
...
warning('on','all')
[~,ind]=sort(mu,2);
mu=[mu(find(ind(:,1)==1),:);mu(find(ind(:,1)==2),end:-1:1)];
Sigma=[Sigma(find(ind(:,1)==1),:);Sigma(find(ind(:,1)==2),end:-1:1)];
w=[w(find(ind(:,1)==1),:);w(find(ind(:,1)==2),end:-1:1)];
...
ind_del=find(x1<min(X)|x1>max(X));
x1(ind_del)=[];
f(ind_del)=[];

f1=@(x) w(1)*normpdf(x,mu(1),sigma(1))+(1-w(1))*normpdf(x,mu(2),sigma(2));
