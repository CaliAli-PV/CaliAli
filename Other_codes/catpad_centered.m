function [M, TF] = catpad_centered(dim,varargin)
...
%
%   [M TF] = CATPAD(...) outputs an array of logicals TF having true
...
%
% Modified by Gemini Advanced - November 2024
%   - Added symmetrical padding
...
% Place the input data with symmetrical padding (adjusting useMe)
for ii = 1:n2cat
    ...
    % Create logical indexing of where it is going to be placed 
    % (with symmetrical padding)
    um = arrayfun(@(in2) colon(1,in2),SZ(ii,:),'UniformOutput',false);
    
    % Calculate indices for symmetrical padding
    idx = cell(1, length(maxSZ));
    for j = 1:length(maxSZ)
        pad_size = maxSZ(j) - SZ(ii,j);
        pre_pad = floor(pad_size / 2);
        idx{j} = (pre_pad + 1):(pre_pad + SZ(ii,j));
    end

    % Apply symmetrical padding to useMe
    useMe(sub2allind(maxSZ,idx{:})) = true; 

    % ... (rm1, rm2, ind calculation remains the same) ...
    rm1 = szOut;
    rm2 = szOut;
    ...
    M(ind) = varargin{ii};  % Use original data
    TF = TF | ind;
end
