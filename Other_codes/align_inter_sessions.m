function [Vout,Po]=align_inter_sessions(P,Va)
...
for k=1:size(P,4)
    temp=Va{k};
    Po(:,:,:,k)=imwarp(P(:,:,:,k),Shifts(:,:,:,k),'FillValues',nan);
    Mo(:,:,k)=imwarp(mean(temp,3),Shifts(:,:,:,k),'FillValues',nan);
    parfor i=1:size(temp,3)
        temp(:,:,i)=imwarp(temp(:,:,i),Shifts(:,:,:,k),'FillValues',nan);
    end
    Vout{1,k}=temp;
end
Po=v2uint8(Po);
...
Mb=squeeze(P(:,:,1,:)); % Mean frame
Vf=squeeze(P(:,:,3,:)); % Blood vessels
for i=1:size(Vf,3)
Vf(:,:,i)=mat2gray(Vf(:,:,i));
end
...
opt{2,1} = struct('stop_criterium',0.001,'imagepad',1.5,'niter',100, 'sigma_fluid',1,...
    'sigma_diffusion',6, 'sigma_i',1,...
    'sigma_x',1, 'do_display',plotme, 'do_plotenergy',plotme);
opt{3,1} = struct('stop_criterium',0.001,'imagepad',1.5,'niter',100, 'sigma_fluid',1,...
    'sigma_diffusion',3, 'sigma_i',1,...
    'sigma_x',1, 'do_display',plotme, 'do_plotenergy',plotme);
opt{4,1} = struct('stop_criterium',0.001,'imagepad',1.5,'niter',20, 'sigma_fluid',1,...
    'sigma_diffusion',3, 'sigma_i',1,...
    'sigma_x',1, 'do_display',plotme, 'do_plotenergy',plotme);
