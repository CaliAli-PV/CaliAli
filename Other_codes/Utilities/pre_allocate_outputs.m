...
function check_files(input_files,tag)
...
for k=1:length(input_files)
    ...
    if ischar(input_files{k})
        ...
        filename = input_files{k}(1);
    end
    [filepath, name] = fileparts(filename);

    % Create an output file name by appending '_det' to the original file name
    if ~contains(name, tag)
        output_file{k} = char(strcat(filepath, filesep, name, tag, '.mat'));
    else
        output_file{k} = char(strcat(filepath, filesep, name, '.mat'));
    end
end
