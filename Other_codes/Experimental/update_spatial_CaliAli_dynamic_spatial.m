...
[A,~,kill]=dynamic_A_NNMF(A_T,weights,[obj.options.d1,obj.options.d2]);
obj.A_batch=A;
obj.A=Ato2d(obj);
obj.delete(find(kill));
end

function out_A=update_spatial_in(obj, use_parallel,max_frame,ix)
...
A_temp=squeeze(obj.A_batch(:,:,ix));

obj.A_prev=A_temp;
IND = sparse(logical(determine_search_location(A_temp, search_method, options)));
...
for mpatch=1:(nr_patch*nc_patch)
    ...
    if strcmpi(bg_model, 'ring')
        ind = find(reshape(mask(:)==1, 1, [])* full(A_temp)>0);
        A_prev{mpatch}= A_temp((mask>0), ind);
...
%% upadte b0
if strcmpi(bg_model, 'ring')
    obj.b0_new = cell2mat(obj.P.Ymean)-obj.reshape(A_temp*mean(obj.C,2), 2); %-obj.reconstruct();
end
