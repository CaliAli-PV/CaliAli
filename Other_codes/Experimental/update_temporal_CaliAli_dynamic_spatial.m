...
function C_raw=update_temporal_in(obj, use_parallel, max_frame,ix)
...
for mpatch=1:(nr_patch*nc_patch)
    ...
    ind = (reshape(mask(:), 1, [])* A_temp>0);
    A{mpatch}= A_temp(logical(mask), ind);
...
aa(aa==0) = 1;
C_raw = bsxfun(@times, C_new, 1./aa);


%% upadte b0
if strcmpi(bg_model, 'ring')
    % fprintf('Update the constant baselines for all pixels..\n');
    obj.b0_new = cell2mat(obj.P.Ymean)-obj.reshape(A_temp*mean(obj.C,2), 2) -obj.reconstruct_b0();
    % fprintf('Done!\n');
end
