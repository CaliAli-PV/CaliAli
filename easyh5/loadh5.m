...
try
  if(nargin>1 && ~isempty(path))
        try
            rootgid = H5G.open(loc, path);  % Try to open 'path' as a group
            [varargout{1:nargout}] = load_one(rootgid, opt);
            H5G.close(rootgid);
        catch
            try 
                % If opening as a group fails, try opening as a dataset
                dataset_id = H5D.open(loc, path);  
                data = H5D.read(dataset_id);
                H5D.close(dataset_id);
                varargout{1} = data;  % Assign the dataset data to the output
                if nargout > 1
                    varargout{2} = struct(); % Or get attributes if needed
                end
            catch ME
                % Handle the case where 'path' is neither a valid group nor dataset
                fprintf(1, 'Error: Invalid path or object type.\n');
