function PcaFilters = runPCA(inputMatrix,  numberPCs, varargin)
    ...
    DFOFsize = size(inputMatrix);
        ...
        if options.usenoisefloor %Determine how many PCs to keep, using an estimate of the noise floor
            ...
            display('finding PCs...')
            try
                [CovEvals PcaTraces PcaFilters nPCs] = calculatePCAInternal(inputMatrix,100,inputMatrixPath,options,DFOFsize);
                % do something
            catch err
                display(repmat('@',1,7))
    ...
    function [CovEvals PcaTraces PcaFilters nPCs] = calculatePCAInternal(inputMatrix,nPCs,inputMatrixPath,options,DFOFsize)
        ...
        inputMatrix = single(reshape(Y,d1*d2,[]));
        ...
        disp('extracting eigenvectors...');drawnow
        ...
        PcaFilters = reshape(PcaFilters,d1,d2,nPCs);
        PcaFilters(PcaFilters<0)=0;
    end
end