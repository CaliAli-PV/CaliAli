function [ai, ci, ind_success, sn] = extract_ac_dendrite(HY, Y, ind_ctr, sz)
...
M=bin_data_max(HY,1,50);
M=M(:,1:2:end);
y0 = M(ind_ctr, :);
w=M/y0;
w = mat2gray(VerticalVesselness2D(reshape(w,nr,nc),0.5:0.1:0.8, [1;1], 0.3,true,0));
I=w>0.02;
CC = bwconncomp(I);
