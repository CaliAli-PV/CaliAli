%% Register two images
function [Mp,sx,sy,vx,vy,im_out,e] = register_in(F,M,opt)
...
% 
% Ip  = ones(ceil(size(I)*scale))*double(min(I,[],'all'));
% lim = bsxfun(@plus, floor(size(I)*(scale-1)/2), [[1 1];size(I)]); % image limits
% Ip(lim(1):lim(2),lim(3):lim(4)) = I;                              % padded image
% I = Ip;
sc=flip(round(size(I)*(scale-1)./2));
lim=[sc+1;sc+size(I)];
I=double(padarray(I,sc, "replicate"));
end
