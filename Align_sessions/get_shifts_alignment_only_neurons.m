function [Shifts,P,Mask]=get_shifts_alignment_only_neurons(P)
...
parfor i=1:size(b,1)
    img=Proj(i,:);
    % get transformations:
    [t_T,t_loc_c,t_glob_c,t_Tb,t_loc_cb,t_glob_cb]=get_transformations(img{1},img{2});
...
for i=1:n
    T{i,i}=T{1, 2}*0;
    LocW(:,:,i)=median(cat(3,loc_w{i,:}),3);
    temp=glob_w(i,:);
    temp(i)=[];
    globW(i)=mean(temp);
end
lobW=globW./sum(globW);
% LocW=LocW./sum(LocW,3);
for i=1:size(LocW,3)
LocW(:,:,i) = imgaussfilt(LocW(:,:,i),8, 'FilterSize',91);
end
LocW=LocW+1;
LocW=mat2gray(LocW./sum(LocW,3)).^2;

for i=1:size(LocW,3)
    LocW(:,:,i) = adapthisteq(LocW(:,:,i));
end
LocW=LocW./sum(LocW,3);
if size(LocW,3)==2
LocW=LocW*0+0.5;
