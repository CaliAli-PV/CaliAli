%% Register two images
function [Mp,sx,sy,vx,vy,im_out,e] = register_in_uncompressible(F,M,opt)
if nargin<3;  opt = struct();  end;
if ~isfield(opt,'sigma_fluid');      opt.sigma_fluid     = 1.0;              end % regularize update      field
if ~isfield(opt,'sigma_diffusion');  opt.sigma_diffusion = 1.0;              end % regularize deformation field
if ~isfield(opt,'sigma_i');          opt.sigma_i         = 1.0;              end % weight on similarity term
if ~isfield(opt,'sigma_x');          opt.sigma_x         = 1.0;              end  % (maximal step)
if ~isfield(opt,'niter');            opt.niter           = 250;              end
if ~isfield(opt,'vx');               opt.vx              = zeros(size(M));   end
if ~isfield(opt,'vy');               opt.vy              = zeros(size(M));   end
if ~isfield(opt,'stop_criterium');   opt.stop_criterium  = 0.001;            end
...
if ~isfield(opt,'incompressibility');         opt.incompressibility   = 1;                end
im_out={};
%% padded image
...
for iter=1:opt.niter
    ...
    % [a,b] = compute_hodge_decompositon(cat(3,vx,vy));
    % vx=b(:,:,1);
    % vy=b(:,:,2);
    ...
    if opt.do_display
        ...
        subplot(2,4,8); showgrid(sx*10,sy*10,4,lim); title('Transformation');
        im_out{5,iter}=sx;
        im_out{6,iter}=sy;
        im_out{7,iter}=lim;
        drawnow;
